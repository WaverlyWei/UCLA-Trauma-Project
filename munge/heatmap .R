...
testHeatmap<-function(dat, annotation) {    
  annotation_latino <- get_annotation("percentpop_under65")
  annotation_pbpl <- get_annotation("percentpop_19plus")
  annotation_under5 <- get_annotation("totalhouse_est" )
...
NeighborhoodHeatmap<-function(dat, annotation) {    
  ...
  sampleColors1 <- mapVarToColor("YlOrRd",annotation_under65)
  sampleColors2 <- mapVarToColor("YlOrRd",annotation_19plus)
  sampleColors3 <- mapVarToColor("YlOrRd",annotation_est)
  ...
  colnames(cols.1)=c("percentpop_under65","percentpop_19plus","totalhouse_est")
  cols.2 <- cbind(sampleColors4,sampleColors5,sampleColors6)
  colnames(cols.2)=c("percentpop_under20to24","percentpop_feamle","percentpop_black")
...
# PCA
# --------------------
features <- dat.2[,c(3,4,25,46:48,72:77,79,81,83,86:98)]
feat.pca <- prcomp(features,center = T,scale = T)
## plot cumulative sum
vars <- apply(feat.pca$x, 2, var)  
props <- vars / sum(vars)
cusum <- cumsum(props)
# scree plot 
plot(cusum,main='PCA Scree Plot')

# 6 features should be ok 
top <- 6
principal.vectors <- as.matrix(feat.pca$rotation)
principal.vectors <- principal.vectors[,1:top]
principal.vectors <- abs(principal.vectors)
# order variables within each PC with decreasing order 
ordered.pc <- apply(principal.vectors,2, 
                     function(x) rownames(data.frame(sort(x,decreasing = T))))
# find the feature with max coorelation with each PC
top.features <- unique(ordered.pc[1,])
